...
\begin{minipage}{0.5\linewidth}		
		...
		\begin{CountingDefinition}[Key attributes~\cite{5myths}]{def:5myths}
			...
			\begin{itemize}[noitemsep,topsep=0pt]
				...
				\item Radical replacements are often not necessary (disruptiveness)
			\end{itemize}
...
\begin{CountingDefinition}[Key attributes~\cite{venkat}]{def:venkat}
		...
		\begin{itemize}[noitemsep,topsep=0pt]
			\item Incremental steps to improve processes (presented as levels)
		\end{itemize}
...
\end{minipage}	\begin{minipage}{0.5\linewidth}
	...
	\begin{CountingDefinition}[Key attributes~\cite{DTOT}]{def:DTOT}
		...
		\textit{Digital transformation versus IT-enabled transformation}~\cite{DTOT}
